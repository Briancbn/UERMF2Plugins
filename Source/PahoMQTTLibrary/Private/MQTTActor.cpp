...
static int PahoMessageArrived(void* Context, char* TopicName, int TopicLen, MQTTClient_message* Message)
{
    return AMQTTActor::OnMessageArrived(Context, TopicName, TopicLen, (void*)Message);
}
    ...
    bool bCleanSession)
{
    ...
    // Create MQTT client
    int rc = MQTTClient_create(&Client, BrokerAddr.c_str(), ClientId.c_str(),
    ...
    MQTTClient_setCallbacks(mqttClient, nullptr, OnConnectionLost, PahoMessageArrived, OnDeliveryComplete);
    ...
    bIsConnected = true;
    LastError = "";
    UE_LOG(LogTemp, Log, TEXT("Successfully connected to MQTT broker: %s"), *BrokerAddress);
    return true;
}
...
bool AMQTTActor::UnsubscribeFromMQTTTopic(const FString& Topic)
{
    ...
    if (rc != MQTTCLIENT_SUCCESS)
    {
        LastError = FString::Printf(TEXT("Failed to unsubscribe from topic: %d"), rc);
        UE_LOG(LogTemp, Error, TEXT("%s"), *LastError);
        return false;
    }
...
FString AMQTTActor::GetMQTTLastError()
{
    return LastError;
}
...
int AMQTTActor::OnMessageArrived(void* Context, char* TopicName, int TopicLen, void* Message)
{
