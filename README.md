...
3. Search for "VDAClientPlugin"

   ![VDAClient Plugin in UE5 Editor](Resources/Plugin.png)

4. Check the box next to **VDAClientPlugin** to enable it
5. Click **Restart Now** when prompted (or just restart after enabling it)

## Usage

After restarting your Unreal Engine Editor, you should be able to see the plugin in UE5's Content Editor.


1. To add the VDA5050Client component into an actor, click on "Add" under the "Components" tab in the Actor Blueprint and search for "VDAClient".

    ![Add Component](Resources/AddComponent.png)
    
    Upon succesfully adding the component to an actor, the VDAClient component will be visible under the "Components" tab.
    
    ![Client Component under Component Tab](Resources/ClientComponent.png)

2. Under the "Details" tab of the component, configure the following properties:
...
## Events and Blueprint functions available:

### Events
Bindable event dispatchers that notify the owning actor of VDA5050 events. Can be added into the event graph by right-clicking the `VDAClient` in the "Components" tab and hovering over "Add Event". Events handle incoming orders and connection status.

![Adding Event to Event Graph](Resources/AddEvent.png)
...
#### Blueprint Functions
Callable functions for controlling the VDA5050 client from Blueprint. Functions can be accessed by left-clicking and dragging the VDAClient variable pin onto the event graph:

![VDAClient Function List](Resources/VDAClientFunctionList.png)
...
1. Example of the VDAClient component being used with AIMoveTo:

    ![Example of VDAClient Blueprint Implementation](Resources/EventGraph.png)